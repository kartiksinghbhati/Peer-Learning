    ...
    if (userData.token && course.id) {
      ...
      await fetch(`${G_API}/courses/${course.id}/courseWork`, { //fetch all the assignments from classrooom and store it in assignments using setAllAssignments
        ...
        .then((res) => {
          ...
          const pastDueCoursework = [];

          for(const coursework of res.courseWork) {
            if(coursework.dueDate){
              const dueDate = new Date(coursework.dueDate.year, coursework.dueDate.month - 1, coursework.dueDate.day, coursework.dueTime.hours);

              if(dueDate < currentDate){
                pastDueCoursework.push(coursework);
              }
            }
          }
          //console.log(pastDueCoursework);

          setAllAssignments(pastDueCoursework);
